...
public class UserDeviceInfoServiceImpl extends ServiceImpl<UserDeviceInfoMapper, UserDeviceInfo> implements UserDeviceInfoService {
    ...
    public void createInfo(CreateUserDeviceInfoForm form) {
        ...
        userDeviceInfo.setUserNo(users.getUserNo());
        userDeviceInfo.setMobile(users.getMobile());
        userDeviceInfo.setAppId(form.getAppId());
        userDeviceInfo.setProjectId(form.getProjectId());
        userDeviceInfo.setSceneId(form.getSceneId());
        userDeviceInfo.setAppVersion(form.getAppVersion());
        userDeviceInfo.setOsVersion(form.getOsVersion());
        userDeviceInfo.setPlatform(form.getPlatform());
        userDeviceInfo.setModel(form.getModel());
        userDeviceInfo.setManufacture(form.getManufacture());
    ...
    public UserDeviceInfoListDTO getList(String appId, String userNo, String projectId, String sceneId, Integer lastDays) {
        ...
        QueryWrapper<UserDeviceInfo> modelQueryWrapper = Wrappers.<UserDeviceInfo>query()
                ...
                .eq("app_id", appId)
                .apply(true, "DATE_SUB(CURDATE(), INTERVAL " + lastDays + " DAY) <= date(created_at)")
                .groupBy("model");
        ...
        modelEntities.forEach(e -> {
            ...
            deviceItemDTO.setTotal((long) Integer.parseInt(e.get("count").toString()));
            deviceItemDTO.setName(e.get("model").toString());
        ...
        manufactureEntities.forEach(e -> {
            DeviceItemDTO deviceItemDTO = new DeviceItemDTO();
            deviceItemDTO.setTotal((long) Integer.parseInt(e.get("count").toString()));
            deviceItemDTO.setName(e.get("manufacture").toString());
        ...
        osVersionEntities.forEach(e -> {
            DeviceItemDTO deviceItemDTO = new DeviceItemDTO();
            deviceItemDTO.setTotal((long) Integer.parseInt(e.get("count").toString()));
            deviceItemDTO.setName(e.get("os_version").toString());
